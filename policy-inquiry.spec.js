...
async function fillVisibleInputs(page, screenName, values) {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    console.log(`Filled input ${i + 1} for ${screenName} with <masked>`);
  }
}
...
async function runInquiryScreen(page, appFrame, screen) {
  ...
  try {
    ...
    await captureMaskedScreenshot(page, {
...
async function applicationCredentialLogin(page) {
  ...
  await captureMaskedScreenshot(page, { path: `${SCREENSHOT_DIR}/03-after-credential-login.png`, fullPage: true });
